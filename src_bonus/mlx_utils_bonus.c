...
void	projection(t_pixel *img, t_map *map, t_camera *camera)
{
	...
	while (i < i_max)
	{
		...
		img[i].z = map->map[i].z * camera->z_divisor * camera->f;
		img[i].h = map->map[i].z;
...
int	render(t_vars *var)
{
	...
	if (var->camera.method == ISOMERIC)
		isomeric(var->img, &var->camera, var->map.m_h * var->map.m_w);
	else if (var->camera.method == PARAL_XY)
		parallel_xy(var->img, &var->camera, var->map.m_h * var->map.m_w);
	else if (var->camera.method == PARAL_XZ)
		parallel_xz(var->img, &var->camera, var->map.m_h * var->map.m_w);
	else if (var->camera.method == PARAL_YZ)
		parallel_yz(var->img, &var->camera, var->map.m_h * var->map.m_w);
	img = var->img;
